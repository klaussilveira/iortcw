...
void CL_PlayCinematic_f( void ) {
	...
	if ( CL_handle >= 0 ) {
		// decode frames until we get a video frame (buf != NULL) or cinematic ends
		// can't use CIN_RunCinematic here because its time-based gating
		// may prevent frame decoding when elapsed time is too small
		while ( cinTable[CL_handle].buf == NULL && cinTable[CL_handle].status == FMV_PLAY ) {
			RoQInterrupt();
		}
	}
}
