...
static keyNum_t IN_TranslateSDLToQ3Key( SDL_Keysym *keysym, qboolean down )
{
	...
	if( IN_IsConsoleKey( key, 0 ) )
	{
		...
		// Toggle console directly since ET doesn't have K_CONSOLE
		key = 0;
	}
...
static qboolean KeyToAxisAndSign(int keynum, int *outAxis, int *outSign)
{
	...
	*outSign = 0;
	...
	// Direct axis mapping without configurable cvars:
	// Axis 0: side (left/right)
	// Axis 1: forward/back
	// Axis 2: up/down
	// Axis 3: yaw (turn left/right)
	// Axis 4: pitch (look up/down)
...
static void IN_GamepadMove( void )
{
	...
	// check buttons - map gamepad buttons to K_JOY1+
	...
	for (i = 0; i < SDL_CONTROLLER_AXIS_MAX; i++)
	{
		...
		if (axis != oldAxis)
		{
			// Map axes to direction keys: left stick = arrows, right stick = joy17-20, triggers = joy21-22
			const int negMap[SDL_CONTROLLER_AXIS_MAX] = { K_LEFTARROW,  K_UPARROW,   K_JOY19, K_JOY17, 0, 0 };
			const int posMap[SDL_CONTROLLER_AXIS_MAX] = { K_RIGHTARROW, K_DOWNARROW,  K_JOY20, K_JOY18, K_JOY21, K_JOY22 };
			...
			if (!posAnalog && posKey && oldAxis > 0 && axis <= 0)
				Sys_QueEvent(in_eventTime, SE_KEY, posKey, qfalse, 0, NULL);

			// negative to positive/neutral -> keyup
			if (!negAnalog && negKey && oldAxis < 0 && axis >= 0)
				Sys_QueEvent(in_eventTime, SE_KEY, negKey, qfalse, 0, NULL);

			// negative/neutral to positive -> keydown
			if (!posAnalog && posKey && oldAxis <= 0 && axis > 0)
				Sys_QueEvent(in_eventTime, SE_KEY, posKey, qtrue, 0, NULL);

			// positive/neutral to negative -> keydown
			if (!negAnalog && negKey && oldAxis >= 0 && axis < 0)
				Sys_QueEvent(in_eventTime, SE_KEY, negKey, qtrue, 0, NULL);
	...
	if (in_joystickUseAnalog->integer)
	{
		for (i = 0; i < MAX_JOYSTICK_AXIS; i++)
		{
			if (translatedAxesSet[i])
				Sys_QueEvent(in_eventTime, SE_JOYSTICK_AXIS, i, translatedAxes[i], 0, NULL);
		}
	}
}
...
static void IN_JoyMove( void )
{
	...
	if (total > 0)
	{
		...
		if (balldx || balldy)
		{
			...
			if (abs(balldy) > 1)
				balldy *= 2;
			Sys_QueEvent( in_eventTime, SE_MOUSE, balldx, balldy, 0, NULL );
		}
	}
		...
		for( i = 0; i < 4; i++ ) {
			if( ((Uint8 *)&hats)[i] != ((Uint8 *)&stick_state.oldhats)[i] ) {
				...
				switch( ((Uint8 *)&stick_state.oldhats)[i] ) {
					case SDL_HAT_UP:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 0], qfalse, 0, NULL );
						break;
					case SDL_HAT_RIGHT:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 1], qfalse, 0, NULL );
						break;
					case SDL_HAT_DOWN:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 2], qfalse, 0, NULL );
						break;
					case SDL_HAT_LEFT:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 3], qfalse, 0, NULL );
						break;
				...
				switch( ((Uint8 *)&hats)[i] ) {
					case SDL_HAT_UP:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 0], qtrue, 0, NULL );
						break;
					case SDL_HAT_RIGHT:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 1], qtrue, 0, NULL );
						break;
					case SDL_HAT_DOWN:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 2], qtrue, 0, NULL );
						break;
					case SDL_HAT_LEFT:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 3], qtrue, 0, NULL );
						break;
					case SDL_HAT_RIGHTUP:
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 0], qtrue, 0, NULL );
						Sys_QueEvent( in_eventTime, SE_KEY, hat_keys[4*i + 1], qtrue, 0, NULL );
	...
	if (axes != stick_state.oldaxes)
	{
		for( i = 0; i < 16; i++ ) {
			if( ( axes & ( 1 << i ) ) && !( stick_state.oldaxes & ( 1 << i ) ) ) {
				Sys_QueEvent( in_eventTime, SE_KEY, joy_keys[i], qtrue, 0, NULL );
			}

			if( !( axes & ( 1 << i ) ) && ( stick_state.oldaxes & ( 1 << i ) ) ) {
				Sys_QueEvent( in_eventTime, SE_KEY, joy_keys[i], qfalse, 0, NULL );
			}
		}
	}
			...
			case SDL_MOUSEMOTION:
				if( mouseActive )
				{
					if( !e.motion.xrel && !e.motion.yrel )
						break;
					Sys_QueEvent( in_eventTime, SE_MOUSE, e.motion.xrel, e.motion.yrel, 0, NULL );
				}
				break;
			...
			case SDL_MOUSEBUTTONUP:
				{
					...
					Sys_QueEvent( in_eventTime, SE_KEY, b,
...
===============
*/
void IN_Init( SDL_Window *windowData )
{
	int appState;
