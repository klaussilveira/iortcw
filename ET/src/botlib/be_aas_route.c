...
void AAS_UpdateAreaRoutingCache(aas_routingcache_t* areacache)
{
    ...
    while (updateliststart) {
        ...
        for (i = 0, revlink = revreach->first; revlink; revlink = revlink->next, i++) {
            ...
            //
            if (aasworld->areatraveltimes[nextareanum] && (!areacache->traveltimes[clusterareanum] || areacache->traveltimes[clusterareanum] > t)) {
            ...
            if (!besttraveltime || besttraveltime > t) {
                ...
                if (aasworld->areavisibility[nextareanum]) {
                    ...
                    if (!AAS_AreaVisible(enemyareanum, nextareanum)) { // now last of all, check that this area is a safe hiding spot
                        if ((aasworld->visCache[nextareanum] == 2) || (!aasworld->visCache[nextareanum] && !botimport.BotVisibleFromPos(enemyorigin, enemynum, aasworld->areawaypoints[nextareanum], srcnum, qfalse))) {
...
int AAS_ListAreasInRange(vec3_t srcpos, int srcarea, float range, int travelflags, vec3_t* outareas, int maxareas)
{
    ...
    while (updateliststart) {
        ...
        for (i = 0; i < numreach; i++, reach++) {
            ...
            if (!(aasworld->areasettings[reach->areanum].areaflags & AREA_LADDER) && !(aasworld->areasettings[reach->areanum].areaflags & AREA_DISABLED) && (AAS_AreaVisible(srcarea, nextareanum))) {
