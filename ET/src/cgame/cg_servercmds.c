    ...
    if (weaponMask != 0) {
        ...
        for (i = WS_KNIFE; i < WS_MAX; i++) {
            if (weaponMask & (1 << i)) {
                ...
                if (nShots > 0 || nHits > 0) {
                    Q_strcat(strName, sizeof(strName), va("%5.1f %4d/%-4d ", ((nShots == 0) ? 0.0 : (float)(nHits * 100.0 / (float)nShots)), nHits, nShots));
                } else {
    ...
    if (!dwWeaponMask) {
        ...
        for (i = WS_KNIFE; i < WS_MAX; i++) {
            if (dwWeaponMask & (1 << i)) {
                ...
                if (atts > 0 || hits > 0) {
                    fHasStats = qtrue;
                    Q_strcat(strName, sizeof(strName), va("^7%5.1f ^5%4d/%-4d ", ((atts == 0) ? 0.0 : (float)(hits * 100.0 / (float)atts)), hits, atts));
                } else {
                    Q_strcat(strName, sizeof(strName), va("                "));
                    if (kills > 0 || deaths > 0) {
                        fHasStats = qtrue;
