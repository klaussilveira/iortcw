...
static void CG_CalculateWeaponPosition(vec3_t origin, vec3_t angles)
{
    ...
    if (!cg.renderingThirdPerson && (cg.predictedPlayerState.weapon == WP_MORTAR_SET || cg.predictedPlayerState.weapon == WP_MOBILE_MG42_SET) && cg.predictedPlayerState.weaponstate != WEAPON_RAISING) {
...
void CG_AltWeapon_f(void)
{
    ...
    if ((cg.snap->ps.weaponstate == WEAPON_RAISING || cg.snap->ps.weaponstate == WEAPON_DROPPING) && ((original == WP_GPG40 || num == WP_GPG40 || original == WP_M7 || num == WP_M7) || (original == WP_SILENCER || num == WP_SILENCER || original == WP_SILENCED_COLT || num == WP_SILENCED_COLT) || (original == WP_AKIMBO_SILENCEDCOLT || num == WP_AKIMBO_SILENCEDCOLT || original == WP_AKIMBO_SILENCEDLUGER || num == WP_AKIMBO_SILENCEDLUGER) || (original == WP_MORTAR_SET || num == WP_MORTAR_SET) || (original == WP_MOBILE_MG42_SET || num == WP_MOBILE_MG42_SET))) {
        return;
    }
...
void CG_WeaponBank_f(void)
{
    ...
    // Arnout: don't allow another weapon switch when we're still swapping the gpg40, to prevent animation breaking
    if ((cg.snap->ps.weaponstate == WEAPON_RAISING || cg.snap->ps.weaponstate == WEAPON_DROPPING) && ((curweap == WP_GPG40 || num == WP_GPG40 || curweap == WP_M7 || num == WP_M7) || (curweap == WP_SILENCER || num == WP_SILENCER || curweap == WP_SILENCED_COLT || num == WP_SILENCED_COLT) || (curweap == WP_MORTAR_SET || num == WP_MORTAR_SET))) {
        return;
    }
...
void CG_FireWeapon(centity_t* cent)
{
    ...
    } else if (ent->weapon == WP_GRENADE_LAUNCHER || ent->weapon == WP_GRENADE_PINEAPPLE || ent->weapon == WP_DYNAMITE || ent->weapon == WP_SMOKE_MARKER
    ...
    case WP_K43_SCOPE:
        ...
        if (weapon == WP_FG42SCOPE || weapon == WP_GARAND_SCOPE || weapon == WP_K43_SCOPE || (Distance(cg.refdef_current->vieworg, origin) < 384)) {
            if (clientNum) {
                ...
                // max made the bullet mark alpha (he'll make everything in the game out of 1024 textures, all with alpha blend funcs yet...)
...
//%		1,1,1,1, qtrue, 15+random()*10, qfalse, cg_bloodTime.integer * 1000 );
        ...
        if (!fromweap || cg_entities[sourceEntityNum].currentState.eFlags & EF_MG42_ACTIVE || cg_entities[sourceEntityNum].currentState.eFlags & EF_MOUNTEDTANK) { // mounted
            fromweap = WP_MP40;
        }
