...
void CG_mvWindowOverlay(int pID, float b_x, float b_y, float b_w, float b_h, float s, int wState, qboolean fSelected)
{
    ...
    ((ci->weaponState == WSTATE_SWITCH) ? &colorWhite : (ci->weaponState == WSTATE_FIRE) ? &colorRed
                                                                                         : &colorYellow));
...
void CG_mvOverlayClientUpdate(int pID, int index)
{
    ...
    if (w != NULL) {
        strcpy(cg.mvOverlay[index].info, va("%s%s%2d", strClassHighlights[cg.mvOverlay[index].classID * 2], (w == cg.mvCurrentMainview) ? "*" : "", pID));
    } else {
