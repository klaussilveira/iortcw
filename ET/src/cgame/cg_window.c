...
void CG_cursorUpdate(void)
{
    ...
    for (i = wh->numActiveWindows - 1; i >= 0; i--) {
        ...
        if ((w->effects & WFX_MULTIVIEW) && w != cg.mvCurrentMainview) {
            ...
            if (!fFound && (((w->mvInfo & MV_SELECTED) && fSelect) || (!fSelect && nx >= w->x && nx < w->x + w->w && ny >= w->y && ny < w->y + w->h))) {
    ...
    // General boundary check
        ...
        for (i = TEAM_AXIS; i <= TEAM_ALLIES; i++) {
            ...
            if (cgs.cursorX >= nx && cgs.cursorY >= ny && cgs.cursorX < MVINFO_RIGHT && cgs.cursorY < ny + (cg.mvTotalTeam[i] * (MVINFO_TEXTSIZE + 1))) {
