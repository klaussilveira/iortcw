    ...
    for (i = 0, cv = cvarTable; i < cvarTableSize; i++, cv++) {
        if (cv->vmCvar) {
            ...
            if (cv->modificationCount != cv->vmCvar->modificationCount) {
                ...
                if (cv->vmCvar == &cg_autoAction || cv->vmCvar == &cg_autoReload || cv->vmCvar == &int_cl_timenudge || cv->vmCvar == &int_cl_maxpackets || cv->vmCvar == &cg_autoactivate || cv->vmCvar == &cg_predictItems) {
                    fSetFlags = qtrue;
...
void CG_setClientFlags(void)
{
    ...
    trap_Cvar_Set("cg_uinfo", va("%d %d %d",
                              // Client Flags
                              (((cg_autoReload.integer > 0) ? CGF_AUTORELOAD : 0) | ((cg_autoAction.integer & AA_STATSDUMP) ? CGF_STATSDUMP : 0) | ((cg_autoactivate.integer > 0) ? CGF_AUTOACTIVATE : 0) | ((cg_predictItems.integer > 0) ? CGF_PREDICTITEMS : 0)
...
void CG_nameCleanFilename(const char* pszIn, char* pszOut, unsigned int dwOutSize)
{
    ...
    while (*pszIn && dwCurrLength < dwOutSize) {
        ...
        if (*pszIn == '\\' || *pszIn == '/' || *pszIn == ':' || *pszIn == '"' || *pszIn == '*' || *pszIn == '?' || *pszIn == '<' || *pszIn == '>' || *pszIn == '|' || *pszIn == '.') {
            pszIn++;
            dwCurrLength++;
            continue;
        }
...
static void CG_RegisterSounds(void)
{
    ...
    for (i = 0; i < 2; i++) {
        cgs.media.grenadebounce[FOOTSTEP_NORMAL][i] = cgs.media.grenadebounce[FOOTSTEP_GRAVEL][i] = cgs.media.grenadebounce[FOOTSTEP_SPLASH][i] = trap_S_RegisterSound(va("sound/weapons/grenade/bounce_hard%i.wav", i + 1), qfalse);

        cgs.media.grenadebounce[FOOTSTEP_METAL][i] = cgs.media.grenadebounce[FOOTSTEP_ROOF][i] = trap_S_RegisterSound(va("sound/weapons/grenade/bounce_metal%i.wav", i + 1), qfalse);

        cgs.media.grenadebounce[FOOTSTEP_WOOD][i] = trap_S_RegisterSound(va("sound/weapons/grenade/bounce_wood%i.wav", i + 1), qfalse);

        cgs.media.grenadebounce[FOOTSTEP_GRASS][i] = cgs.media.grenadebounce[FOOTSTEP_SNOW][i] = cgs.media.grenadebounce[FOOTSTEP_CARPET][i] = trap_S_RegisterSound(va("sound/weapons/grenade/bounce_soft%i.wav", i + 1), qfalse);
    }
