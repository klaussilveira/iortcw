...
static float CG_DrawTimer(float y)
{
    ...
    rt = (cgs.gametype != GT_WOLF_LMS && cg_drawReinforcementTime.integer > 0) ? va("^F%d%s", CG_CalculateReinfTime(qfalse), ((cgs.timelimit <= 0.0f) ? "" : " ")) : "";
...
static float CG_ScanForCrosshairEntity(float* zChange, qboolean* hitClient)
{
    ...
    *zChange = trace.endpos[2] - start[2];
    ...
    if (trace.entityNum >= MAX_CLIENTS) {
        ...
        if ((cg_entities[trace.entityNum].currentState.eType == ET_MOVER && cg_entities[trace.entityNum].currentState.effect1Time) || cg_entities[trace.entityNum].currentState.eType == ET_CONSTRUCTIBLE_MARKER) {
...
void CG_CheckForCursorHints(void)
{
    ...
    if (trace.entityNum >= MAX_CLIENTS && (tracent->currentState.powerups == STATE_INVISIBLE || tracent->currentState.powerups == STATE_UNDERCONSTRUCTION)) {
        return;
    }
...
static void CG_DrawCrosshairNames(void)
{
    ...
    if (cg.crosshairClientNum > MAX_CLIENTS) {
        ...
        if ((cg_entities[cg.crosshairClientNum].currentState.powerups & (1 << PW_OPS_DISGUISED)) && cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR) {
            if (cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR && cgs.clientinfo[cg.snap->ps.clientNum].skill[SK_SIGNALS] >= 4 && cgs.clientinfo[cg.snap->ps.clientNum].cls == PC_FIELDOPS) {
...
static qboolean CG_DrawFollow(void)
{
    ...
    if (cg.snap->ps.pm_flags & PMF_LIMBO) {
        ...
        CG_DrawStringExt(84, 118, va("%s %s [%s]", cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_ALLIES ? rankNames_Allies[cgs.clientinfo[cg.snap->ps.clientNum].rank] : rankNames_Axis[cgs.clientinfo[cg.snap->ps.clientNum].rank], cgs.clientinfo[cg.snap->ps.clientNum].name, BG_ClassnameForNumber(cgs.clientinfo[cg.snap->ps.clientNum].cls)),
...
static void CG_DrawWarmup(void)
{
    ...
    if (!sec) {
        if ((cgs.gamestate == GS_WARMUP && !cg.warmup) || cgs.gamestate == GS_WAITING_FOR_PLAYERS) {
            ...
            if (!cg.demoPlayback && cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR && (!(cg.snap->ps.pm_flags & PMF_FOLLOW) || (cg.snap->ps.pm_flags & PMF_LIMBO))) {
                char str1[32];
...
static void CG_DrawFlashDamage(void)
{
    ...
    if (cg.v_dmg_time > cg.time) {
        ...
        col[3] = 0.7 * (redFlash / 5.0) * ((cg_bloodFlash.value > 1.0) ? 1.0 : (cg_bloodFlash.value < 0.0) ? 0.0
                                                                                                           : cg_bloodFlash.value);
...
static void CG_DrawPlayerStatus(void)
{
    ...
    if (cg.mvTotalClients < 1 && cg_drawWeaponIconFlash.integer == 0) {
        ...
        CG_DrawPlayerWeaponIcon(&rect, (ws != WSTATE_IDLE), ITEM_ALIGN_RIGHT, ((ws == WSTATE_SWITCH) ? &colorWhite : (ws == WSTATE_FIRE) ? &colorRed
                                                                                                                                         : &colorYellow));
    }
