        ...
        } else // if we are close to the flag, get it!
        {
            ...
            && (BotFlagAtBase(level.attackingTeam == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS, &flag) == qtrue)) {
                ...
                if (bs->ainode == AINode_MP_MedicRevive || bs->ainode == AINode_MP_MedicGiveHealth) {
                    ...
                    if (trap_InPVS(bs->origin, g_entities[bs->target_goal.entitynum].r.currentOrigin) && VectorDistanceSquared(bs->origin, g_entities[bs->target_goal.entitynum].r.currentOrigin) < (512 * 512)) {
                        getflag = qfalse;
                    }
                }
                //
        ...
        case BPG_NEWGOAL:
            ...
            if (bestGoal_numCloser >= 0) {
                ...
                if (numCloser == bestGoal_numCloser) {
                    ...
                    if ((findGoals[i].defaultPriority == findGoals[bestGoal_InList].defaultPriority) && (findGoals[i].priority <= findGoals[bestGoal_InList].priority)) {
                        continue;
                    }
                }
            }
...
qboolean BotMP_FindGoal(bot_state_t* bs)
{
...

    {
        ...
        if (closestTime) {
            ...
            // is it for us only?
            if (((trav->spawnflags & 1) && (bs->sess.sessionTeam == TEAM_AXIS)) || ((trav->spawnflags & 2) && (bs->sess.sessionTeam == TEAM_ALLIES))) {
