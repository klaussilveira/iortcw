...
int AINode_MP_AvoidDanger(bot_state_t* bs)
{
    ...
    if (!trav->inuse) {
        ...
        if ((g_entities[bs->client].client->lastConstructibleBlockingWarnEnt != goal.entitynum) || ((level.time - g_entities[bs->client].client->lastConstructibleBlockingWarnTime) > 5000)) {
            bs->ignore_specialgoal_time = 0;
            BotDefaultNode(bs);
            return qfalse;
        }
...
int AINode_MP_GiveAmmo(bot_state_t* bs)
{
    ...
    // reload?
    if ((bs->last_fire != level.time) && (bs->cur_ps.ammoclip[BG_FindClipForWeapon(bs->cur_ps.weapon)] < (int)(0.8 * (GetAmmoTableData(bs->cur_ps.weapon))->maxclip))
            ...
            if (VectorDistanceSquared(bs->origin, goal.origin) > SQR(32)) {
                ...
                if (bs->enemy >= 0) {
                    ...
                    if (bs->enemy >= 0) {
                        // attack and keep moving
                        if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy, NULL)) {
                            if (VectorDistanceSquared(bs->origin, goal.origin) > SQR(128) || VectorDistanceSquared(bs->origin, g_entities[bs->enemy].r.currentOrigin) > SQR(256)) {
                                // choose the best weapon to fight with
                                BotChooseWeapon(bs);
                                // aim at the enemy
                                BotAimAtEnemy(bs);
...
int AINode_MP_DefendTarget(bot_state_t* bs)
{
    ...
    */
    ...
    if (goal.flags & GFL_LEADER) {
        ...
        if (bs->leader > -1) {
            if (!g_entities[bs->leader].inuse || !g_entities[bs->leader].client || (g_entities[bs->leader].client->ps.pm_flags & PMF_LIMBO)) {
                bs->leader = -1;
                BotDefaultNode(bs);
                return qfalse;
            } else if (bs->sess.playerType == PC_MEDIC) {
                if (g_entities[bs->leader].health <= 0 && trap_AAS_PointAreaNum(g_entities[bs->leader].r.currentOrigin) && BotGoalForEntity(bs, bs->leader, &bs->target_goal, BGU_MEDIUM)) {
                    // revive
                    g_entities[bs->target_goal.entitynum].missionLevel = level.time + 3000;
                    AIEnter_MP_MedicRevive(bs);
                    return qfalse;
                } else if (BotHealthScale(bs->leader) <= 0.7 && trap_AAS_PointAreaNum(g_entities[bs->leader].r.currentOrigin) && BotGoalForEntity(bs, bs->leader, &bs->target_goal, BGU_MEDIUM)) { // health stock?
                    // make this our goal
                    g_entities[bs->target_goal.entitynum].missionLevel = level.time + 3000;
                    AIEnter_MP_MedicGiveHealth(bs);
                    return qfalse;
            ...
            //
            if (!g_entities[bs->leader].inuse || g_entities[bs->leader].health <= 0 || VectorDistanceSquared(g_entities[bs->leader].r.currentOrigin, bs->origin) > SQR(MAX_BOTLEADER_DIST)) {
                bs->leader = -1;
            }
        }
        //
            ...
            if (bs->enemy >= 0) {
                ...
                if (bs->enemy >= 0) {
                    ...
                    if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy, NULL)) {
                        ...
                        if (!BotCarryingFlag(bs->target_goal.entitynum) && ((bs->weaponnum == WP_LUGER) || (bs->weaponnum == WP_COLT) || (bs->weaponnum == WP_MOBILE_MG42))) {
                            AIEnter_MP_Battle_Fight(bs);
                            return qfalse;
                        } else {
                            // choose the best weapon to fight with
...
int AINode_MP_TouchTarget(bot_state_t* bs)
{
    ...
    */
    ...
    if ((BotCarryingFlag(bs->client)) && (goal.number < level.time - 8000) && (BotNumTeamMatesWithTarget(bs, bs->client, NULL, -1) < 2) && (trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bs->target_goal.areanum, bs->tfl) > 1500)) {
        if (bs->enemy > -1) {
            ...
            if ((bs->stand_time < level.time - 3000) && (trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bs->target_goal.areanum, bs->tfl) > 600) && (numList = BotNumTeamMatesWithTarget(bs, bs->client, list, MAX_CLIENTS))) {
                // if one of them has a goal infront of us, and they are not near it, we should pause for a bit
                for (i = 0; i < numList; i++) {
                    tbs = &botstates[list[i]];
                    //
                    if ((VectorDistanceSquared(tbs->origin, tbs->defendgoal.origin) < SQR(1500)) && (VectorDistanceSquared(tbs->origin, bs->origin) < SQR(1024)) && (trap_InPVS(tbs->origin, bs->origin)) && (trap_AAS_AreaTravelTimeToGoalArea(tbs->defendgoal.areanum, tbs->defendgoal.origin, goal.areanum, bs->tfl) < trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bs->target_goal.areanum, bs->tfl)) && (trap_AAS_AreaTravelTimeToGoalArea(tbs->areanum, tbs->origin, goal.areanum, bs->tfl) - trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, goal.areanum, bs->tfl))) {
