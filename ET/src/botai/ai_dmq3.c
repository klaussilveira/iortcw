...
passed:
    ...
    if (bs->cur_ps.weapon == WP_GRENADE_LAUNCHER || bs->cur_ps.weapon == WP_GRENADE_PINEAPPLE) {
        if (bs->cur_ps.grenadeTimeLeft && ((bs->inventory[INVENTORY_HEALTH] < 15) || (bs->cur_ps.grenadeTimeLeft < (700 + (int)(4000.0 * (1.0 - aimskill)))))) {
            // release grenade
        } else {
            // hold onto it
...
void BotRecordPain(int client, int enemy, int mod)
{
    ...
    if (bs->defendgoal.entitynum >= level.maxclients) {
        ...
        if (Q_stricmp(targ->classname, "team_CTF_redflag") && Q_stricmp(targ->classname, "team_CTF_blueflag") && Q_stricmp(targ->classname, "trigger_flagonly") && Q_stricmp(targ->classname, "team_WOLF_checkpoint")) {
            return;
        }
