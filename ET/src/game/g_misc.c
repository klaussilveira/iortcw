...
void SP_misc_gamemodel(gentity_t* ent)
{
    ...
    if (ent->spawnflags & 2) {
        ...
        (void)G_SpawnString("reverse", "", &dummy);
...
void landmine_setup(gentity_t* ent)
{
    ...
    if (tr.startsolid || tr.fraction == 1.f || !(tr.surfaceFlags & (SURF_GRASS | SURF_SNOW | SURF_GRAVEL | SURF_LANDMINE)) || (tr.entityNum != ENTITYNUM_WORLD && (!g_entities[tr.entityNum].inuse || g_entities[tr.entityNum].s.eType != ET_CONSTRUCTIBLE))) {
