...
{
    if (weaponNum == WP_AKIMBO_COLT || weaponNum == WP_AKIMBO_SILENCEDCOLT || weaponNum == WP_AKIMBO_LUGER || weaponNum == WP_AKIMBO_SILENCEDLUGER) {
        return qtrue;
    } else {
        return qfalse;
    }
...
void BG_setCrosshair(char* colString, float* col, float alpha, char* cvarName)
{
    ...
    col[1] = 1.0f;
    col[2] = 1.0f;
    col[3] = (alpha > 1.0f) ? 1.0f : (alpha < 0.0f) ? 0.0f
                                                    : alpha;
...
{
    if (weapon == WP_LUGER || weapon == WP_COLT || weapon == WP_MP40 || weapon == WP_THOMPSON || weapon == WP_STEN || weapon == WP_SILENCER || weapon == WP_FG42 || weapon == WP_SILENCED_COLT) {
        return qtrue;
    }
    return qfalse;
}
