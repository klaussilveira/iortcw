...
void Team_FragBonuses(gentity_t* targ, gentity_t* inflictor, gentity_t* attacker)
{
    ...
    if (flag) { // JPW NERVE -- added some more stuff after this fn
        ...
        if ((VectorLengthSquared(v1) < SQR(CTF_TARGET_PROTECT_RADIUS) || VectorLengthSquared(v2) < SQR(CTF_TARGET_PROTECT_RADIUS) || CanDamage(flag, targ->client->ps.origin) || CanDamage(flag, attacker->client->ps.origin)) && attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
...
void G_verifyMatchState(int nTeam)
{
    ...
    if ((level.lastRestartTime + 1000) < level.time && (nTeam == TEAM_ALLIES || nTeam == TEAM_AXIS) && (gs == GS_PLAYING || gs == GS_WARMUP_COUNTDOWN || gs == GS_INTERMISSION)) {
