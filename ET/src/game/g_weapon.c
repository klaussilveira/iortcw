...
static qboolean TryConstructing(gentity_t* ent)
{
    ...
    if (constructible->s.eType == ET_CONSTRUCTIBLE && ent->client->touchingTOI->chain) {
        ...
        if (otherconstructible->s.angles2[0] || otherconstructible->s.angles2[1] || (otherconstructible->count2 && otherconstructible->grenadeFired)) {
            ...
            if (traceEnt->s.teamNum >= 4) {
                ...
                for (i = 0; i < num; i++) {
                    ...
                    if ((hit->s.eType == ET_OID_TRIGGER)) {
                        ...
                        if (((hit->spawnflags & AXIS_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_AXIS)) || ((hit->spawnflags & ALLIED_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_ALLIES))) {
                            // bani
                            //							G_FreeEntity( traceEnt );
                        ...
                        // bani
                        if (((hit->spawnflags & AXIS_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_ALLIES)) || ((hit->spawnflags & ALLIED_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_AXIS))) {
                            enemyObj = qtrue;
                        }
                    }
                }
                ...
                for (i = 0; i < num; i++) {
                    ...
                    if ((hit->s.eType == ET_OID_TRIGGER)) {
                        ...
                        if (!(hit->spawnflags & 128) && (((hit->spawnflags & AXIS_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_ALLIES)) || ((hit->spawnflags & ALLIED_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_AXIS)))) {
                ...
                //  Arnout - first see if the dynamite is planted near a constructable object that can be destroyed
                {
                    ...
                    for (e = 0; e < numListedEntities; e++) {
                        ...
                        if (hit->parent) {
                            ...
                            if ((!(hit->parent->spawnflags & OBJECTIVE_DESTROYED)) && hit->s.teamNum && (hit->s.teamNum == ent->client->sess.sessionTeam)) { // ==, as it's inverse
                                AddScore(traceEnt->parent, WOLF_DYNAMITE_PLANT);                                                                             // give drop score to guy who dropped it
                                if (traceEnt->parent && traceEnt->parent->client) {
                                    G_LogPrintf("Dynamite_Plant: %d\n", (int)(traceEnt->parent - g_entities)); // OSP
                                }
...
qboolean G_PlayerCanBeSeenByOthers(gentity_t* ent)
{
    ...
    for (i = 0, ent2 = g_entities; i < level.maxclients; i++, ent2++) {
        ...
        if (G_VisibleFromBinoculars(ent2, ent, pos[0]) || G_VisibleFromBinoculars(ent2, ent, pos[1]) || G_VisibleFromBinoculars(ent2, ent, pos[2])) {
            return qtrue;
        }
    }
...
void FireWeapon(gentity_t* ent)
{
    ...
    if (ent->client->ps.powerups[PW_OPS_DISGUISED] && ent->s.weapon != WP_SMOKE_BOMB && ent->s.weapon != WP_SATCHEL && ent->s.weapon != WP_SATCHEL_DET) {
        if (!(ent->s.weapon == WP_KNIFE || ent->s.weapon == WP_STEN || ent->s.weapon == WP_SILENCER || ent->s.weapon == WP_SILENCED_COLT || ent->s.weapon == WP_AKIMBO_SILENCEDCOLT || ent->s.weapon == WP_AKIMBO_SILENCEDLUGER || ent->s.weapon == WP_K43 || ent->s.weapon == WP_K43_SCOPE || ent->s.weapon == WP_GARAND || ent->s.weapon == WP_GRENADE_LAUNCHER || ent->s.weapon == WP_GRENADE_PINEAPPLE || ent->s.weapon == WP_GARAND_SCOPE)) {
