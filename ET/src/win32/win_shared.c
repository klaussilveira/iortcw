...
static void CPUID(int func, unsigned regs[4])
{
    ...
    __asm mov eax, func __asm cpuid __asm mov regEAX, eax __asm mov regEBX, ebx __asm mov regECX, ecx __asm mov regEDX, edx
...
float Sys_RealGetCPUSpeed(void)
{
    ...
    } while ((tries < 3 || tries < 20) && ((abs(3 * freq1 - total) > 3) || (abs(3 * freq2 - total) > 3) || (abs(3 * freq3 - total) > 3)));
