...
bot_moveresult_t AICast_CombatMove(cast_state_t* cs, int tfl)
{
    ...
    if (cs->combatGoalTime > level.time) {
        ...
        } else if (!AICast_CheckAttack(cs, cs->bs->enemy, qfalse) && AICast_VisibleFromPos(cs->vislist[cs->bs->enemy].visible_pos, cs->bs->enemy, cs->bs->origin, cs->entityNum, qfalse)) {
            cs->combatGoalTime = 0;
            cs->combatSpotAttackCount = cs->startAttackCount;
        }
