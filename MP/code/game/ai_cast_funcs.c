...
char* AIFunc_Idle(cast_state_t* cs)
{
    ...
    if (!(cs->leaderNum >= 0) && cs->lastEnemy >= 0 && g_entities[cs->lastEnemy].health > 0 && cs->vislist[cs->lastEnemy].real_visible_timestamp < level.time - 5000 && cs->takeCoverTime < level.time - 5000) {
...
char* AIFunc_BattleMG42(cast_state_t* cs)
{
    ...
    if (bs->enemy < 0 || !AICast_CheckAttack(cs, bs->enemy, qfalse) || (fabs(AngleDifference(angles[YAW], mg42->s.angles[YAW])) > mg42->harc) || (angles[PITCH] < 0 && angles[PITCH] + 5 < -mg42->varc) || (angles[PITCH] > 0 && angles[PITCH] - 5 > 5.0)) {
        qboolean shouldAttack;
