...
void Team_FragBonuses(gentity_t* targ, gentity_t* inflictor, gentity_t* attacker)
{
    ...
    if (flag) { // JPW NERVE -- added some more stuff after this fn
        ...
        if ((VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS || VectorLength(v2) < CTF_TARGET_PROTECT_RADIUS || CanDamage(flag, targ->client->ps.origin) || CanDamage(flag, attacker->client->ps.origin)) && attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
...
void Team_ReturnFlagSound(gentity_t* ent, int team)
{
    ...
    te->s.eventParm = G_SoundIndex(team == TEAM_RED ? "sound/multiplayer/axis/g-objective_secure.wav" : "sound/multiplayer/allies/a-objective_secure.wav");
