...
static Symbol spillee(Symbol set, unsigned mask[], Node here)
{
    ...
    if (!set->x.wildcard)
        ...
        for (i = 31; i >= 0; i--) {
            ...
            if (
            ri != NULL && ri->x.lastuse && (ri->x.regnode->mask & tmask[ri->x.regnode->set] & mask[ri->x.regnode->set])) {
